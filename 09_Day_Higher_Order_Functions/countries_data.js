...
function categorizeCountries(pattern) {
  const categorized = countries.filter((country) =>
    country.name.toLowerCase().includes(pattern.toLowerCase())
  );
  return categorized;
}

//console.log(categorizeCountries("republic"));

// 4.
function firstLetters() {
  const letterCount = {};
  countries.forEach((country) => {
    let firstLetter = country.name.slice(0, 1).toUpperCase();
...
function getLastTenCountries() {
  const lastTenCountries = countries.slice(countries.length - 10);
...
// 7.
function getMostUsedInitialLetter() {
  let initialLettersCount = Object.entries(firstLetters());
  let mostUsed = "";
  let mostUsedCount = 0;
  initialLettersCount.forEach(([key, value]) => {
  ...
  return `Most used initial letter for country name is ${mostUsed} at a count of ${mostUsedCount}.`;
}

//console.log(getMostUsedInitialLetter());
...
function mostSpokenLanguages(countries, count) {
  ...
  const languagesCountEntries = Object.entries(languagesCount).sort(
    (a, b) => b[1] - a[1]
  );
  const languages = languagesCountEntries.slice(0, count);
  const languagesFormated = [];
  languages.forEach((language) => {
    languagesFormated.push({
      language: language[0],
      count: language[1],
    });
  });
...
function mostPopulatedCountries(countries, count) {
  ...
  const mostPopulated = countries.slice(0, count);
  const mostPopulatedArray = [];
  mostPopulated.forEach((country) => {
    mostPopulatedArray.push({
      country: country.name,
      population: country.population,
    });
  });
  return mostPopulatedArray;
}
